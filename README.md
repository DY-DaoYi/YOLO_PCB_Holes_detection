...
> 3. **（赠品）数据集**：本项目使用 [Roboflow PCB Holes Dataset](https://universe.roboflow.com/rf-projects/pcb-holes/dataset/4) 的开源数据集。
...
- **🇨🇳 全中文支持**: 从界面菜单到检测框标签（如"缺孔"等缺陷），全部采用中文显示，符合国内用户习惯。
...
为了让系统能够识别 PCB 图像中的缺陷目标，你需要加载训练好的 YOLO 模型文件（`.pt`）。
...
2. **放置模型**：将你训练得到的 `best.pt` 文件复制到 `models/` 文件夹中（建议重命名为清晰的名称，如 `yolo11m_pcb_holes.pt`）。
...
本项目使用 [Roboflow PCB Holes Dataset](https://universe.roboflow.com/rf-projects/pcb-holes/dataset/4) 开源数据集，主要用于检测 **PCB 孔位缺陷**。
...
2. **训练规模**:
  ...
  - **训练轮数**: 100 Epochs / 模型
3. **实际耗时与成本**:
  - *总耗时*: 01:13:21 (约 1.22 小时)
  - *云服务器费率*: ¥9.35 / 小时
  - **总算力成本**: **1.22 小时 x ¥9.35 ≈ ¥11.43**
