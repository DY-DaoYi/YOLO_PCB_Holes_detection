...
def generate_pdf_report(summary_data, detail_data, output_path):
    ...
    # 1. 标题页
    elements.append(Paragraph("PCB孔检测报告", title_style))
    ...
    if total_class_counts:
        elements.append(Paragraph("二、各类别缺陷统计", header_style))
        class_stats_data = [["缺陷类别", "检测数量"]]
    ...
    for item in detail_data:
        # 文件标题
        elements.append(Paragraph(f"文件名: {item['filename']} (缺孔数: {item['count']})", header_style))
